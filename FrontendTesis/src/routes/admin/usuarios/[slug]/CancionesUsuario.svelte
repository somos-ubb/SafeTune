<script>
    ...
    let { cancionesUsuario, usuarioCanciones } = $props();
    ...
    let userInputFilter = $state('both'); // 'both', 'yes', 'no'

    // Derived filtered cancionesUsuario
    let filteredCanciones = $derived(cancionesUsuario.filter(cancion => {
        return (
            ...
            (userInputFilter === 'both' || (userInputFilter === 'yes' && cancion.input===1) || (userInputFilter === 'no' && cancion.input===0) || (userInputFilter === 'null' && cancion.input===null))
        );
    }));

    // Derived artista suggestions based on search input
    let artistaSuggestions = $derived([...new Set(cancionesUsuario.map(c => c.artista))]
...
<div class="container mx-auto p-4 flex">
    ...
    <aside class="w-1/4 p-4 bg-gray-100 rounded-lg mr-4">
        ...
        <!-- User Input Filter -->
        <div class="mb-4 mt-4">
            <p class="block text-gray-700">Input de Usuario:</p>
            <select bind:value={userInputFilter} class="mt-1 block w-full p-2 border border-gray-300 rounded-lg">
                <option value="both">Mostrar Ambos</option>
                <option value="yes">Mostrar con Input violento</option>
                <option value="no">Mostrar con Input no violento</option>
                <option value="null">Mostrar sin Input</option>
            </select>
        </div>
    ...
    <!-- Songs List Section -->
    <div class="w-3/4">
        <h1 class="text-2xl font-bold mb-4">Lista de Canciones de {usuarioCanciones}</h1>
        <button class="bg-violet-300 rounded" onclick={()=>downloadJSONAsCSV(cancionesUsuario, `canciones-${usuarioCanciones}`)}>Descargar CSV de canciones de {usuarioCanciones}</button>
        <ul class="space-y-4">
            {#each filteredCanciones as cancion}
                <li class="bg-white shadow-md rounded-lg p-4">
                    ...
                    <p class="text-gray-600">Tiempo escuchado: {cancion.timeListen/1000} segundos</p>
                    <p class="text-gray-600">Según usuario es Violento: 
                        {#if cancion.input === 1}
                            Sí
                        {:else if cancion.input === 0}
                            No
                        {:else}
                            Sin Input
                        {/if}
